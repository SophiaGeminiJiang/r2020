...
```{r}
doc_frame <- data.frame(matrix(0,nrow = 2125,ncol = 6))
colnames(doc_frame) <- c("num","month","begin_or_not","adopted_before","contacts_before","total_contacts")
doc_frame$num <- rep(1:nrow(ckm_nodes),each = 17)
doc_frame$month <- rep(1:17,times = nrow(ckm_nodes))
doc_frame$begin_or_not <- doc_frame$month == rep(ckm_nodes$adoption_date, each=17)
doc_frame$adopted_before <- doc_frame$month > rep(ckm_nodes$adoption_date, each=17)
con_before <- function(t){
  i <- doc_frame$num[t]
  j <- doc_frame$month[t]
  s <- sum(ckm_network[row.names(ckm_network) == as.character(i),] == 1 & ckm_nodes$adoption_date < j)
  return(s)
}
doc_frame$contacts_before = sapply(seq(1,nrow(doc_frame),by = 1),con_before)
con_before2 <- function(t){
  i <- doc_frame$num[t]
  j <- doc_frame$month[t]
  s <- sum(ckm_network[row.names(ckm_network) == as.character(i),] == 1 & ckm_nodes$adoption_date <= j)
  return(s)
}
doc_frame$total_contacts = sapply(seq(1,nrow(doc_frame),by = 1),con_before2)
```
3.  Let
    \[
   ...
   We suppose that $p_k$ and $q_k$ are the same for all months.
    ...
    From `ckm_network`, no doctors have contacts more than 20. So there should be no more than 21 values of $k$ for which we can estimate $p_k$ and $q_k$ directly from the data.
...
```{r}
prob <- vector(mode = "numeric", length = 21)
for (i in 0:20){
  prob[i+1] <- sum(doc_frame$"begin_or_not" == TRUE &
    doc_frame$contacts_before == i)/sum(doc_frame$contacts_before == i)
}
plot(x = seq(0,20,by = 1), y = prob,xlab = "number", ylab = "p_k")
```
...
```{r}
prob2 <- vector(mode = "numeric", length = 21)
for (i in 0:20){
  prob2[i + 1] <- sum(doc_frame$"begin_or_not" == TRUE &
    doc_frame$total_contacts == i)/sum(doc_frame$total_contacts == i)
}
plot(x = seq(0,20,by = 1), y = prob2,xlab = "number", ylab = "q_k")
```
...
The values are `a = 0.06075572` and `b = -0.004004967`
```{r}
ls <- lsfit(seq(0,20,by = 1),prob)
a <- ls[[1]][[1]]
b <- ls[[1]][[2]]
linear_pred <- seq(0,20,by = 1)* b + a
```
...
This model implies that the probability would rise but in a much slower way when adding one more adoptee friend on a doctor. The values are `a = -2.6265` and `b = -0.1333`
```
expmodel <- nls(prob ~ exp(a + b* (seq(0, 20, by = 1)))/(1 + exp(a + b * (seq(0, 20, by = 1)))), start = list(a = 0.1,b = 0.1))
```
```{r}
a <- -2.6265
b <- -0.1333
exp_pred <- exp(a + b*(seq(0, 20, by = 1)))/(1 + exp(a + b*(seq(0, 20, by = 1))))
```
...
From the plot, the expmodel is more accurate.
```{r}
pred_frame <- data.frame(cbind(seq(0,20,by = 1), prob, linear_pred, exp_pred))

colnames(pred_frame)[[1]] <- "num"
ggplot(data = pred_frame) + geom_point(aes(x = num, y = prob)) +
  labs(x = "k" ,y = "probability") +
  geom_line(aes(x = num, y = linear_pred),col = "blue",size = 1.5) + geom_line(aes(x = num, y = exp_pred), col = "red", size = 1.5)
```
